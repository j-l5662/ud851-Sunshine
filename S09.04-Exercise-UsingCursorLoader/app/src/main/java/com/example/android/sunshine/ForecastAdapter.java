...
//      done (8) Display the summary that you created above
        ...
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext,mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID)));
        long high = mCursor.getLong(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
        long low = mCursor.getLong(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        String highandlowtemps = SunshineWeatherUtils.formatHighLows(mContext,high,low);

        forecastAdapterViewHolder.weatherSummary.setText( date+ "-" + description+ "-" + highandlowtemps);
