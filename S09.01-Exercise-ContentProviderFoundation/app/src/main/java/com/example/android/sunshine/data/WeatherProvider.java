...
//  done (7) Instantiate a static UriMatcher using the buildUriMatcher method
    private static final UriMatcher sUriMatcher = new WeatherProvider().buildUriMatcher();
...
//      done (9) Handle queries on both the weather and weather with date URI
        ...
        switch(match) {
            case CODE_WEATHER:
                retCursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME,projection,selection,
                        selectionArgs,null,null,sortOrder);
                break;
            case CODE_WEATHER_WITH_DATE:
                String id = uri.getPathSegments().get(1);
                String mSelection = WeatherContract.WeatherEntry.COLUMN_DATE + "_id=?";
                String[] mSelectionArgs = new String[]{id};
                retCursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME,projection,mSelection,
